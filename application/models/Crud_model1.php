...
class Crud_model extends CI_Model { 
    ...
    function get_students($class_id){
        $select = "select * from student where class_id = '".$class_id."' order by name asc";
		$query = $this->db->query($select);
		//$this->db->get_where('student', array('class_id' => $class_id));
        return $query->result_array();
    }
   ...
   function send_student_score_model(){
    ...
    function check_availability_for_student($online_exam_id){
		...
		array('online_exam_id' => $online_exam_id, 'student_id' => $this->session->userdata('login_user_id')))->row_array();
