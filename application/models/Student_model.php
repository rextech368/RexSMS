...
// The function below update student category //
    ...
    function createNewStudent(){
        ...
        $page_data = array(
            ...
            'roll'          => html_escape($this->input->post('roll_prefix')) . html_escape($this->input->post('roll')), // Combine prefix with generated roll
		...
		if($transport_id == ''){
			$page_data['transport_id'] = 0;
		}else{
			$page_data['transport_id'] = html_escape($this->input->post('transport_id'));
		}
		
		$dormitory_id = html_escape($this->input->post('dormitory_id'));
		if($transport_id == ''){
			$page_data['dormitory_id'] = 0;
		}else{
			$page_data['dormitory_id'] = html_escape($this->input->post('dormitory_id'));
		}
		
		$house_id = html_escape($this->input->post('house_id'));
		if($house_id == ''){
			$page_data['house_id'] = 0;
		}else{
			$page_data['house_id'] = html_escape($this->input->post('house_id'));
		}
		
		$student_category_id = html_escape($this->input->post('student_category_id'));
		if($student_category_id == ''){
			$page_data['student_category_id'] = 0;
		}else{
			$page_data['student_category_id'] = html_escape($this->input->post('student_category_id'));
		}
    ...
    function get_students_by_session($session) {
        $this->db->select('*');
    ...
    function deleteNewStudent($param2){
        ...
        if (count($threads) > 0) {
            foreach ($threads as $row) {
                ...
                if (($sender[0] == 'student' && $sender[1] == $param2) || ($receiver[0] == 'student' && $receiver[1] == $param2)) {
